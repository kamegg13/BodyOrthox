...
describe('createAnalysis', () => {
  ...
  it('accepts boundary values for confidence (0 and 1)', () => {
    expect(() => createAnalysis({ patientId: 'p1', angles: validAngles, confidenceScore: 0 })).not.toThrow();
    expect(() => createAnalysis({ patientId: 'p1', angles: validAngles, confidenceScore: 1 })).not.toThrow();
  });
});
...
describe('confidenceLabel', () => {
  ...
  it('returns "Moyenne" for score >= 0.60 and < 0.85', () => {
    expect(confidenceLabel(0.60)).toBe('Moyenne');
  ...
  it('returns "Faible" for score < 0.60', () => {
    ...
    expect(confidenceLabel(0.50)).toBe('Faible');
