...
interface PatientsActions {
  ...
  updatePatient(id: string, partial: Partial<Pick<Patient, 'name' | 'dateOfBirth' | 'morphologicalProfile'>>): Promise<void>;
  ...
  immer((set, get) => ({
    ...
    async loadPatients() {
      ...
      try {
        ...
        set(state => { state.patients = patients; state.isLoading = false; });
      } catch (error) {
        set(state => {
