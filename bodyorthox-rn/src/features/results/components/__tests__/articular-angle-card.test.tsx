...
describe("ArticularAngleCard", () => {
  describe("knee joint normal", () => {
    it("renders joint name", () => {
      const assessment = assessAngle("knee", 175);
      const { getByText } = render(
        <ArticularAngleCard assessment={assessment} />,
      );
    ...
    it("renders angle value", () => {
      ...
      expect(getByText("175.0°")).toBeTruthy();
    });
  ...
  describe("ankle joint", () => {
    it("renders ankle label", () => {
      const assessment = assessAngle("ankle", 175);
      const { getByText } = render(
        <ArticularAngleCard assessment={assessment} />,
      );
  ...
  it("has accessibility label", () => {
    ...
    expect(card.props.accessibilityLabel).toContain("175.0");
  });
  ...
  it("shows deviation amount for abnormal values", () => {
    const assessment = assessAngle("knee", 190); // > normalMax of 180, deviation = 10
    const { getByText } = render(
      <ArticularAngleCard assessment={assessment} />,
    );
